...
> This is how the build log of our application might look like. Route `/api` is marked as static, meaning its result will be cached and returned every time the endpoint is called.
> This behavior may not be desirable in all cases, and we can easily change it.
...
> In this way we create tables that we will then use within our application. For more information about all the possible data types supported in SQlite, we recommend [drizzle documentation](https://orm.drizzle.team/docs/column-types/sqlite).
