...
<MdxImage
  src={FRONTEND_DEVELOPER}
  alt="Frontend developer responsibilities across design, UI implementation and browser behavior"
/>
...
<MdxImage
  src={STATE_OF_JS}
  alt="State of JavaScript survey results"
/>
...
<MdxImage src={NODEJS} alt="Node.js logo" />
...
Install the current **LTS** release - Next.js 16 requires at least Node.js 20.9, and the tooling around it (ESLint 10 among others) has already moved past Node 18. Check what you have with `node --version`. If you need to switch between versions on the same machine, use a version manager such as [nvm](https://github.com/nvm-sh/nvm) or [fnm](https://github.com/Schniz/fnm), and pin the version for a project in an `.nvmrc` file.

### Bun

[https://bun.sh/](https://bun.sh/)

<MdxImage src={BUN} alt="Bun logo" />
...
Like Prettier, you install it via npm. The configuration lives in an `eslint.config.mjs` file, which exports an array of config objects - the so-called **flat config**. It was introduced in ESLint 9, became the default there, and since ESLint 10 it is the only supported format. If you open an older project (or an older tutorial) and find an `.eslintrc.json` or `.eslintrc.js` file with an `extends` array, you are looking at the previous format.

For a Next.js application we install `eslint` together with `eslint-config-next`, which already bundles the recommended rules for React, React Hooks and Next.js itself:
...
export default defineConfig([
  ...nextVitals,
  ...nextTs,
  // turns off the ESLint rules that would fight with Prettier
  prettier,
  {
    rules: {
      'no-console': 'off'
...
<MdxComment>
  The `core-web-vitals` config raises the rules that affect Core Web Vitals from
  warnings to errors, and `eslint-config-next/typescript` adds the rules from
  [typescript-eslint](https://typescript-eslint.io/). Both are what
  `create-next-app` sets up for a new TypeScript project.
</MdxComment>

The linter is run through the ESLint CLI - `npx eslint .`, or `pnpm exec eslint .`. Next.js used to wrap it in a `next lint` command, but that command was **removed in Next.js 16**, and so was the `eslint` option in `next.config.ts`. If you meet `next lint` in a tutorial, replace it with `eslint .`.

<MdxImage
  src={ESLINT_ERRORS_DEMO}
  alt="Editor showing ESLint error messages in JavaScript code"
/>
