...
With Cache Components enabled, prefetching works per route rather than per link: Next.js prefetches the route's static shell (its prerendered part), and the request-specific parts stream in after the navigation. Next.js v16 also deduplicates shared layouts across prefetches and only fetches the parts that are not in the cache yet, which means you may see more, but much smaller, prefetch requests than in v15.
...
The same applies to `searchParams`, which is a Promise as well, and to the `params` of a layout, a route handler and `generateMetadata`.
