...
<MdxComment>
  Since Next.js v15, `params` is a **Promise** and has to be awaited. The
  `RouteContext` type is generated by Next.js from your actual route structure,
  so you do not have to write the params type by hand.
</MdxComment>
...
<MdxComment>
  This is how the build log of our application might look like for the first
  example. Route `/api` is marked as static, meaning that its response was
  produced during the build and is served from the prerendered output. The
  second example, which reads the current time, would be listed as dynamic
  instead and executed on every request.
</MdxComment>
...
<MdxComment>
  Note the second snippet also fixes a real bug that was in the original
  example: `Response.json({ ... }), { ... }` is a comma expression, so the
  second object was silently thrown away and never became response headers.
</MdxComment>
...
The route segment options allow you to configure the behavior of a `page`, `layout` or `route` by exporting a specially named variable from the file. In Next.js 16 with Cache Components enabled, these are the options that are still available:

```ts
export const runtime = 'nodejs'; // 'edge' is deprecated
export const maxDuration = number;
export const preferredRegion = 'auto'; // deprecated
```

- `runtime` selects the JavaScript runtime the segment runs in. `'nodejs'` is the default and the only one we use in this course; the `'edge'` runtime is deprecated.
- `maxDuration` sets the maximum execution time of the segment. The actual limit is enforced by the deployment platform (Vercel, for example).
- `preferredRegion` used to hint where the segment should be executed. It is deprecated.
...
### Legacy caching configs

Older tutorials will show you these four exports, which used to be the main way of controlling caching:

```ts
// ❌ removed when `cacheComponents` is enabled (Next.js 16)

export const dynamic = 'auto' | 'force-dynamic' | 'error' | 'force-static';
export const dynamicParams = true | false;
export const revalidate = false | 0 | number;
export const fetchCache = 'auto' | 'default-cache' | 'only-cache'; // and more
```

In Next.js 16, **all four are removed when Cache Components is enabled**, which is the case in this course application. They still exist in projects that have not opted into Cache Components, where they behave as before:

- `dynamic` forced a whole route to be either static (`'force-static'`) or rendered per request (`'force-dynamic'`).
- `revalidate` set how often a statically generated route is regenerated.
- `fetchCache` overrode the `cache` option of every `fetch` in the segment. This was always an escape hatch and is best avoided.
- `dynamicParams` controlled whether a dynamic segment not returned by `generateStaticParams` is generated on request (`true`, the default) or returns a 404 (`false`).

The reason they are gone is that they operate on a whole route at once, which contradicts the whole point of Cache Components: caching is now decided **per component and per function** with `'use cache'`, and everything else either streams behind a `<Suspense>` boundary or is plain static output. Instead of "make this page dynamic", you now say "this piece of data is cached for an hour, this piece is read from the request".

> `export const experimental_ppr = true` is also gone in Next.js 16 - partial prerendering is not experimental anymore, it is what Cache Components does by default.
