# Async, TanStack Query, Forms, Zod

In this lesson, we'll look at asynchronous functions and how to work with them in React. Next, we'll take a look at the TanStack Query library to make working with data easier. We'll examine the Zod library and explain why we need runtime data validation. Finally, we'll look at forms and the React Hook Form library.

## Async

Asynchronous programming is a key component of modern JavaScript and React, allowing efficient processing of operations that may take a long time - for example, retrieving data from an API. The oldest approach was to pass a callback that runs once the operation finishes, which nests badly as soon as one asynchronous step depends on another. ES6 (2015) introduced promises and `.then()` chains, and ES2017 added `async`/`await`, which is what we use today.
...
<MdxComment>
  A promise chain. This is how data was fetched between ES6 and the arrival of
  `async`/`await`.
</MdxComment>
...
It is common in React that we need to set the state of a component in an asynchronous function. For example, when retrieving data from the API, we want to put the `loading` state to `true` before we start loading the data and then to `false` again when the data is loaded. As we know from the lesson on state, calling a setter does not change the value immediately - it schedules an update, and React batches all the updates scheduled during one task into a single re-render.

An `await` splits the function into exactly such tasks. Everything up to the first `await` runs in the task that started the function, so those updates are rendered together as soon as the function yields at the `await`. The code after the `await` continues in a later task, so the updates it schedules are rendered together in their own pass. Since React 18 this is the same automatic batching that applies everywhere else - the `await` does not disable it, it only marks where one batch ends and the next begins.
...
- `setLoading(true)` is called first, but it does not produce an immediate rerender. It only schedules a state update. The `loading` variable in this run of `onClick` stays `false`.

- Execution reaches the `await`. At this point the function yields control, and the browser is free to do other work.

- React now processes the scheduled update, re-renders the component and the browser paints "Loading...".

- After 2 seconds the promise resolves and the rest of `onClick` continues in a new task. `setLoading(false)` schedules another update.

- The function ends, React processes that update and the browser displays "Not loading" again.
...
<MdxComment>
  ...
  by displaying it to the user or logging it to the server. Without the
  try/catch block the promise would simply be rejected: the browser reports an
  unhandled rejection in the console, the user gets no feedback at all, and
  everything we expected to happen after the call never runs.
</MdxComment>
...
<MdxComment>
  Note that the client is created inside the component with `useState`, not next
  to it at module level. In an application that renders on the server - which
  ours does, as we will see with Next.js - a module-level client would be
  created once per server process and therefore **shared between requests and
  between users**. Creating it in `useState` gives every browser its own client,
  and the initializer function makes sure it is created only on the first
  render.
</MdxComment>
...
For example, I have one queryKey to retrieve all users, which we define as `[users]`. We also have a custom `queryKey` for each user, which is defined as `["users", userId]`. In case we need to invalidate all users, we just need to call `queryClient.invalidateQueries({ queryKey: ['users'] })`, which invalidates every query key that starts with `['users']`, including all the keys defined as `['users', userId]`.
...
The most commonly used properties that `useQuery` returns are `data`, `isPending`, `isError` and `refetch`.

- The `data` is the actual data we retrieved in the `queryFn` we defined. It is typed as whatever we return in `queryFn`. It may also be undefined if the data has not yet been retrieved.

- The `isPending` and `isError` booleans tell us if the data is still being fetched or if there was an error while getting the data. These two booleans are important for TypeScript and inferring the data type. If `isPending` is false and `isError` is false as well, TypeScript will correctly narrow the data type, and we no longer need to check if `data` is undefined.
...
- `mutateAsync` is an alternative to `mutate`, except that here we have control over the promise returned by `mutationFn`. In general, it is more recommended to stick with `mutate` with the proviso that we pass `onSuccess` or `onError` callbacks.
...
function useCreateCategoryMutation() {
  return useMutation({
    mutationFn: createCategory,
    onSuccess: () => {
      // Necessary to return a Promise, if we want to wait for the Promise to be resolved!
      return wait(1000);
    }
  });
}
...
<MdxComment>
  In this example we have a controlled form: the input's value is held in a
  state, and every change updates that state through the `setName` function.
</MdxComment>
...
<MdxComment>
  In this example, we have an uncontrolled form: the value lives in the DOM and
  we only read it from the event once the form is submitted. Note that
  TypeScript is not happy with this - `e.target` is typed as a generic
  `EventTarget`. Typing the handler as `(e: FormEvent<HTMLFormElement>)` and
  reading `e.currentTarget.name.value` instead makes it work, as we saw in the
  lecture on React.
</MdxComment>
...
In all cases, if we had multiple fields, we would have to perform validation over individual values, react dynamically to changes in individual field values, etc., and it would be tough to scale these solutions. That's why libraries for implementing forms were created. The most widely used one today is React Hook Form, which we will look at in this lecture; the other name worth knowing is TanStack Form, a newer library from the authors of TanStack Query and TanStack Table with first-class TypeScript inference. (You will still find plenty of tutorials recommending Formik - it is effectively unmaintained, so do not start a new project with it.)
...
These two options are commonly used. The `defaultValues` option defines the default values for the fields in the form. The `resolver` is the function that runs when the form is submitted and validates all the fields. RHF is often used with `zod`, where we define a form schema using Zod and then pass this resolver as an option to `useForm` using the `zodResolver` adapter.
...
<MdxComment>
  ...
  `formSchema` schema as a string.
</MdxComment>

**register, handleSubmit**

We need two functions from the `form` instance to create a basic form - `handleSubmit` and `register`.
...
<MdxComment>
  Here we wrapped the entire form in a `FormProvider`, which allows any child to
  reach the form instance through `useFormContext`. The custom `CustomFormInput`
  component does not register its field with `register`; it uses the
  `Controller` component together with the `control` object obtained from the
  context. It also watches the value of `name`, so it can change its own
  behaviour based on another field.
</MdxComment>
