...
import { homeworkRepositories } from '@/db/schema/homework-repository';
import {
  createGithubClient,
  githubOrganization,
  GithubSetupError
...
import {
  type HomeworkRepositoryInput,
  type HomeworkRepositoryResult,
  type HomeworkRepositoryType
} from '../schema';
...
const withHomeworkRepositoryContext = async (
  ...
  { lectureId, studentId }: HomeworkRepositoryInput,
  operation: (
  ...
  try {
    ...
    record = await homeworkRepositoryRepository.get(lectureId, studentId);
    ...
    if (record) {
      try {
        await homeworkRepositoryRepository.update(record.id, {
          lastError: message
        });
      } catch {
